x_axis= 1:1:1443;
laplace_noise_error_table = readtable("logs/CDP_Estimation_error_laplace_noise.xlsx","ReadVariableNames",true); 
optimal_noise_error_table = readtable("logs/CDP_Estimation_error_optimal_noise.xlsx","ReadVariableNames",true);
...
opt_noise_txt=sprintf('Error using the DP-noise (Trunc. Optimal) CDP "Mean=%0.4f (m)"',opt_noise_error_mean);
laplace_noise_txt=sprintf('Error using the DP-noise (Trunc. Laplace) CDP "Mean=%0.4f (m)"',laplace_noise_error_mean);
