...
configfile = 'config/nodepositions_nesl_mobile_2';
...
for i=1:length(known_anchors)
    ...
    nm.nodes{ aidx }.var_p = nm.nodes{ aidx }.true_p;
end
...
measurement_history=[meas1.nodei meas1.nodej];
...
while (t_last - t_start) < t_stop
    ...
    measurement_history=[measurement_history;meas.nodei meas.nodej];
    if isempty(meas)
        break;
    end
    ...
    h = @(s) nm.measurementFcndp(s, meas);
    % start dp
%     f = nm.processFcndp(s, dt_ref);
%     h = nm.measurementFcndp(s, meas);
    %end dp
