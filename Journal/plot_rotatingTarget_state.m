...
for j=1:length(p_history)
    ...
    xyz_tru = target_location_history(j,:);
    ...
    xyz_array = [xyz_array; t xyz_tru(1) xyz_tru(2) xyz_tru(3)];
    err_array = [err_array; t xyz_err];
    sup_array = [sup_array; t sup_est(1) sup_est(2) sup_est(3)];
    infi_array = [infi_array; t infi_est(1) infi_est(2) infi_est(3)];
    rad_array =[rad_array;t 0.5*(sup_est(1)-infi_est(1)) 0.5*(sup_est(2)-infi_est(2)) 0.5*(sup_est(3)-infi_est(3))];
end

rad_total = [rad_array(1:length(rad_array),2);rad_array(1:length(rad_array),3);rad_array(1:length(rad_array),4)];
...
t_history_plot=k_history;
p_history_plot=p_history;
targetLoc_history_plot=target_location_history;
pSupremumAll_history_plot=pSupremumAll_history;
pInfimumAll_history_plot=pInfimumAll_history;
if(plotting)
    ...
    plot(xyz_array(8:downsample:end,1) - xyz_array(1,1), xyz_array(8:downsample:end,statenum), '-', 'Color', [0 0 1]);
    
    hold on
    plot(xyz_array(8:downsample:end,1) - xyz_array(1,1), sup_array(8:downsample:end,statenum), '-x', 'Color', [0 0 0]);
    plot(xyz_array(8:downsample:end,1) - xyz_array(1,1), infi_array(8:downsample:end,statenum), '-o', 'Color', [1 0 0]);
    xlabel('Time Step');
    ylabel('State x[1] (m)');
    grid on;
   % xlim([0 1000]);
