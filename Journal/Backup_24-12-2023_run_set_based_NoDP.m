...
% to use centralized EKF
    ...
    zdp = meas.vectorizedp_optimal_noise(nm.meascnt);
...
if meas.nodei==10 || meas.nodej==10
     %[s, P] = lkf(f, s, P, h, z, dt_ref*Q, R,nm,meas);
      ...
      szono = intersectZonoZonoTest(f,s,P,h,z,dt_ref*Q,R,nm.nodes{9}.x_zonotope,method,meas.getmeasurementzonotope,meas.getdpzonotope);
      szonodp = intersectZonoZonoTest(f,sdp,P,h,zdp,dt_ref*Q,R,szonodp,method,meas.getmeasurementzonotope,meas.getdpzonotope);
      s = center(szono);
      sdp = center(szonodp);
      P = generators(szono);
      %P = P1(:,1:3);
...
errordp=target_location_history - sdp_history;
normerror=zeros(length(error),1);
normerrordp=zeros(length(errordp),1);
for i=1:length(error)
    normerror(i)=norm(error(i,:));
    normerrordp(i)=norm(errordp(i,:));
end
T=table(error,normerror);
filename = 'logs/NoDP_Estimation_error.xlsx';
writetable(T,filename,'Sheet',1,'Range','A1:D1444')
plot(x_axis',normerror);
hold on
plot(x_axis',normerrordp);
%dp p_history{1,1};
...
legend({'Error without the differential privacy noise','Error in presence of the differential privacy noise (optimal noise)'}, 'Orientation', 'vertical', 'Location', 'NE');
