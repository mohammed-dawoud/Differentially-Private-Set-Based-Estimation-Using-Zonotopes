...
if meas1.nodei==10||meas1.nodej==10
measurement_history=[measurement_history;meas1];
end
...
while (t_last - t_start) < t_stop
    ...
    measurement_history=[measurement_history;meas];
    end
    if isempty(meas)
        break;
    end
...
% to use centralized EKF
    ...
    zdp = meas.vectorizedp_optimal_noise_CDP(nm.meascnt);
...
filename = 'logs/CDP_Estimation_error_optimal_noise--.xlsx';
...
saveName ='cache/temp';
save('cache/temp/CDP_opt', 'nm', 'k_history', 'p_history','target_location_history' ,'t_history','pSupremumAll_history','pInfimumAll_history');
