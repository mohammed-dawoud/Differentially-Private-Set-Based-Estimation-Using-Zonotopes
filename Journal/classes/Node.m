...
%dp         function s = getState(obj)
%dp             s = [obj.state_p; obj.state_clkofst; obj.state_clkbias];
%dp         end
        ...
        function setState(obj, si)
            if(obj.mobile) %dp start
            obj.state_p = si(1:3);
            else 
                obj.state_p = obj.true_p;%si(1:3);
            end %dp end
%dp             obj.state_clkofst = si(4);
%dp             obj.state_clkbias = si(5);
        end
        
        % get the process variance vector
        function q = getProcessVar(obj)
            q = [obj.var_p]; %dp ; obj.var_co; obj.var_cb];
        end
        
        % get the initial variance vector
        function q = getInitialVar(obj)
            q = [obj.vari_p];%dp obj.vari_co; obj.vari_cb];
        end
        
        % set this as the reference node
        function setAsReference(obj)
%dp             obj.state_clkofst = 0;
%dp             obj.state_clkbias = 0 ;
            obj.vari_p = [1e-14; 1e-14; 1e-14 ];
%dp             obj.var_co = 1e-14 ;
%dp             obj.vari_co = 1e-14 ;
%dp             obj.var_cb = 1e-14 ;
%dp             obj.vari_cb = 1e-14 ;
        ...
        %% amr code
        %dp-start modified
        function set_meas(obj,meas,pi,pj,zm,zdp)
            
            obj.destID = meas.getDestId();
        %dp-start-Dawoud
            srcID = meas.getSourceId();
            id = obj.getId();
%             if obj.getId()~=10
%                 Rm = meas.getCovariance();
%                 obj.x_zonotope = zonotope(obj.true_p,diag([Rm;Rm;Rm]));
%             end
            obj.meas_zonotope{obj.zono_index} = meas.getmeasurementzonotope();%dp
            obj.dp_zonotope{obj.zono_index} = meas.getdpzonotope();%dp
        %dp-end-Dawoud

            obj.myh = zm;
            obj.myY = meas.vectorize();
            obj.myYdp = meas.vectorize();
            obj.yl{obj.fill_index} = meas.vectorize();
            obj.Rl{obj.fill_index} = meas.getCovariance();
            obj.ylm{obj.fill_index} = meas.vectorize();
            obj.yldp{obj.fill_index} = meas.vectorize();
            obj.hl{obj.fill_index} = [pj-pi];
       ...
       function efk_part2(obj,fstate,Q,diffEnable)
        ...
        function alg_p1(obj,fstate,algorithm,method,Q)
            ...
            if strcmp(algorithm,'set-membership')
                [obj.eita_zonotope]=intersectZonoZono(obj.x_zonotope,obj.meas_zonotope,obj.dp_zonotope,obj.hl,obj.Rl,obj.yldp,method);%dp
