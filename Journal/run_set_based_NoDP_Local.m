...
%start-dp
previous_walltime = meas1.getTime();
%end-dp
t_last = t_start;
k = 0;
...
% to use centralized EKF
    ...
    zdp = meas.vectorizedp(nm.meascnt);
    hdp = @(s) nm.measurementFcndp(s, meas);
    %dp-start 
    
    %dp-end
      nm.setMeasParameters(meas,pi,pj,zm,zdp);%dp
      
      if walltime == previous_walltime
          count=count+1;
      
      else
          count = 1;
          previous_walltime = walltime;
      end
...
if meas.nodei==10 || meas.nodej==10
     %[s, P] = lkf(f, s, P, h, z, dt_ref*Q, R,nm,meas);
      ...
      szono(count) = intersectZonoZonoTest(f,s,P,h,z,dt_ref*Q,R,nm.nodes{9}.x_zonotope,method,meas.getmeasurementzonotope,meas.getdpzonotope);
      s = center(szono(count));
      P = generators(szono(count));
      %P = P1(:,1:3);
      nm.nodes{9}.x_zonotope=szono(count);
...
filename = 'NoDP_Estimation_error_local_estimation.xlsx';
